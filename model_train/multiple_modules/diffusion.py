...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_in, noise=None):
        ...
        x_sr_64 = torch.nn.functional.interpolate(x_sr_64,scale_factor = 1/4,mode = 'bilinear', align_corners = False)
        x_start_64 = copy.deepcopy(x_start)
        x_start_64 = torch.nn.functional.interpolate(x_start_64,scale_factor = 1/4,mode = 'bilinear', align_corners = False)
        #print(x_start.shape)
        ...
        noise_256 = default(noise, lambda: torch.randn_like(x_start))
        noise_128 = default(noise, lambda: torch.randn_like(x_start_128))
        noise_64 = default(noise, lambda: torch.randn_like(x_start_64))
        x_noisy = self.q_sample(
            x_start=x_start, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=noise)
        x_noisy_128 = self.q_sample(
            x_start=x_start_128, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=noise_128)
        x_noisy_64 = self.q_sample(
            x_start=x_start_64, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=noise_64)

        if not self.conditional:
            x_recon_256 = self.denoise_fn(x_noisy, continuous_sqrt_alpha_cumprod)
        else:
            x_recon_256 = self.denoise_fn(
                torch.cat([x_in['SR'], x_noisy], dim=1), continuous_sqrt_alpha_cumprod)
            x_recon_128 = self.denoise_fn(
                torch.cat([x_sr_128, x_noisy_128], dim=1), continuous_sqrt_alpha_cumprod)
            x_recon_64 = self.denoise_fn(
                torch.cat([x_sr_64, x_noisy_64], dim=1), continuous_sqrt_alpha_cumprod)
        ...
        loss_256 = self.loss_func(noise_256, x_recon_256)
        loss_128 = self.loss_func(noise_128, x_recon_128)
        loss_64 = self.loss_func(noise_64, x_recon_64)
        loss = (loss_256+loss_128+loss_64)/3
