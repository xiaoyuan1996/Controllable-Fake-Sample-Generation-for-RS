...
class GaussianDiffusion(nn.Module):
    ...
    def neibor_16_mul(self, num, size=16):
        a = num // size
        b = num % size
        if b >= 0.5 * size:
            return size * (a + 1)
    ...
    def p_sample_loop(self, x_in, hr_in = None,continous=False,condition_ddim = False,steps = 2000,eta = 1.0):
        ...
        if condition_ddim:
            ...
            seq = [0, 1,2,3,4, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900]
            seq_next = [-1] + seq[:-1]
            print(seq)
            ...
            for idx,(i, j) in tqdm(enumerate(zip(reversed(seq), reversed(seq_next))), desc='sampling loop time step', total=len_seq):
                ...
                if reshape:
                    ...
                    cur_idx = cur_idx if cur_idx < reshape_stage else reshape_stage - 1

                    # print(cur_idx)
                    h, w = hs[cur_idx], ws[cur_idx]
                    # print(h, w)
                    im_resize = Resize([h, w])

                    x_in_tmp = im_resize(x_in)
                    #depth_info_tmp = im_resize(depth_info)
                    x = im_resize(x)

                    et = self.denoise_fn(torch.cat([x_in_tmp,x], dim=1), noise_level)
                else:
